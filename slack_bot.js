...
controller.hears(['korero maori', 'korero Maori'], 'direct_message,direct_mention,mention', function(bot, message) {
    ...
    controller.storage.users.get(message.user, function(err, user) {
        if (user && user.name) {
            bot.reply(message, 'Me korero Maori a  ' + bot.identity.name + ' inaianei?');
        } else {
            bot.startConversation(message, function(err, convo) {
                if (!err) {
                    convo.say('I do not know how to speak Maori.');
                    convo.ask('Which iwi dialect will you teach me?', function(response, convo) {
                        convo.ask('Would you like me to learn `' + response.text + '`?', [
                            {
                                pattern: 'yes',
                                callback: function(response, convo) {
                                    convo.say('Nga mihi')
